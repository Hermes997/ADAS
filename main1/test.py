...
def Detect_distance(car_w, car_x, car_h, car_y, img_cenx):
    ...
    # 실제 차 범퍼 길이, car_w 인식되는 차길이
    car_distance = rw * f / real_w / 2
...
# box1는 이전 프레엠에서 탐지된 물체들, box2는 현재프레임에서 탐지된 물체들
def Change_rec(box1, box2):
    ok = 0
    new_rec = []
    for b2 in box2:
        for b1 in box1:
...
def Previous_check(pbox, lbox, H, W):
    ...
    for i in box:
        if i[0] <= 10 or i[1] <= 10 or i[0] + i[2] > W - 10 or i[1] + i[3] > H - 10:
            box.remove(i)
    return box
...
def Detect_object(source, img, net, CONFIDENCE, THRESHOLD):
    H, W, _ = img.shape
    global p2_width
    global p1_width
    global p2_height
    ...
    global DETECT_CAR
    global prevTime
    global p2_relative_speed
    global p1_relative_speed
    global count
    ...
    blob = cv2.dnn.blobFromImage(source, scalefactor=1 / 255., size=(416, 416), swapRB=True)
    net.setInput(blob)
    output = net.forward()
    ...
    if len(idxs) > 0:
        for i in idxs.flatten():
            ...
            if cap_image_width * 0.4 - y * cap_image_width / cap_image_height * 0.05 < x < cap_image_width * 0.5:
                ...
                # 거리에 따른 위험도가 높은순서대로 빨강, 주황, 초록 색깔로 표시
                if distance < 20:
                    cv2.rectangle(img, pt1=(x, y), pt2=(x + w, y + h), color=(0, 0, 255), thickness=2)
                    cv2.putText(img, distance_str + 'm', (x1 + int(w * 0.3), y1 + 15), font, 1,
                                (0, 0, 255), 1)
                elif distance < 50:
                ...
                # 상대속도, TTC 출력
                if relative_speed >= 0:
                    cv2.putText(img, 'Rel_speed :   ' + final_speed_str + 'km/h', (x1 - 50, y1 - 10), font, 1,
                                (50, 255, 50), 1)
                    cv2.putText(img, 'TTC : far away', (x1 - 30, y1 - 25), font, 1, (50, 255, 50), 1)
                else:
                    time_to_collision = int(distance / ((-relative_speed) * 2))
                    cv2.putText(img, 'Rel_speed : ' + final_speed_str + 'km/h', (x1 - 50, y1 - 10), font, 1,
                                (50, 255, 50), 1)
                    cv2.putText(img, 'TTC : ' + str(time_to_collision) + 'sec', (x1 - 30, y1 - 25), font, 1,
                                (0, 200, 200), 1)
                ...
                p2_height = p1_height
                p1_height = h
                p2_relative_speed = p1_relative_speed
                p1_relative_speed = relative_speed
            ...
            # 측면 차량의 경우
                ...
                if distance < 20:
                    cv2.rectangle(img, pt1=(x, y), pt2=(x + w, y + h), color=(0, 0, 255), thickness=2)
                    cv2.putText(img, distance_str + 'm', (x + int(w * 0.3), y + 15), font, 1,
                                (0, 0, 255), 1)
                elif distance < 50:
                    cv2.rectangle(img, pt1=(x, y), pt2=(x + w, y + h), color=(0, 102, 255), thickness=2)
                    cv2.putText(img, distance_str + 'm', (x + int(w * 0.3), y + 15), font, 1,
    ...
    # 딥러닝 기반 오브젝트 탐지에 실패할 경우 트래킹시도
    for j in Tracker:
        x2, y2, w2, h2 = j

        distance = round(Detect_distance(w2, x2, h2, y2, W / 2))
        distance_str = str(distance)
        if distance < 20:
            cv2.rectangle(img, pt1=(x2, y2), pt2=(x2 + w2, y2 + h2), color=(0, 0, 255), thickness=2)
            cv2.putText(img, "tracking", (x2, y2 + h2 + 10), font, 1, (0, 0, 255), 1)
        elif distance < 50:
            cv2.rectangle(img, pt1=(x2, y2), pt2=(x2 + w2, y2 + h2), color=(0, 102, 255), thickness=2)
            cv2.putText(img, "tracking", (x2, y2 + h2 + 10), font, 1, (0, 102, 255), 1)
        # 위험도가 낮은 초록색범위는 트래킹하지 않음
        else:
            pass

    DETECT_CAR = boxes

    # 현재 트래킹 정보를 저장, 이후 다음 트래킹을 쉽게 하기위한 정보로 쓰임
    if len(boxes) != 0:
        Pcheck += 1
    if Pcheck == 20:
        Prev = Tracker
...
def Decide_lane(lane_image, source):
    ...
    line_image_left = np.zeros((lane_image_gray_height, lane_image_gray_width, 3), lane_image_bgr.dtype)
    line_image_right = np.zeros((lane_image_gray_height, lane_image_gray_width, 3), lane_image_bgr.dtype)
    # 차선 이탈 판단에 이용되는 변수
    departure_count = 0
    ...
    for j in range(10, lane_image_gray_height - 10, 1):
        c = 0
        i1 = 0
        ...
        for i in range(int(lane_image_gray_width * 0.3), int(lane_image_gray_width * 0.05), -1):
            i1 = i
            if int(lane_image_gray[j, i]) >= 50 and 2 < c < 10:
                line_image_left[j, i] = (255, 255, 255)
                cv2.rectangle(return_image, (i - 3, j - 3), (i, j), (255, 0, 0), 1)
                count_lane_left = count_lane_left + 1
                break
        ...
        for k in range(int(lane_image_gray_width * 0.7), int(lane_image_gray_width * 0.95)):
            k1 = k
            if int(lane_image_gray[j, k]) >= 50 and 2 < c < 10:
                line_image_right[j, k] = (255, 255, 255)
                cv2.rectangle(return_image, (k, j), (k + 3, j + 3), (255, 0, 0), 1)
                count_lane_right = count_lane_right + 1
                break

        # 차선이 오른쪽, 왼쪽 한쪽차선만 검출될때, 두쪽 모두 검출될때의 경우로 나누어 동작후 영역 출력
        if i1 != int(lane_image_gray_width * 0.05) + 1 and k1 != int(lane_image_gray_width * 0.95) - 1:
            cv2.rectangle(return_image, (i1, j), (k1, j + 2), (0, 100, 0), 2)
        elif i1 != int(lane_image_gray_width * 0.05) + 1:
            cv2.rectangle(return_image, (i1, j), (i1 + int(lane_image_gray_width * 0.67), j + 2), (0, 100, 0), 2)
        elif k1 != int(lane_image_gray_width * 0.95) - 1:
            cv2.rectangle(return_image, (k1 - int(lane_image_gray_width * 0.67), j), (k1, j + 2), (0, 100, 0), 2)

        # 차선 이탈 판단
        if lane_image_gray_height * 0.7 < j < lane_image_gray_height * 0.85:
            if i1 > lane_image_gray_width * 0.25 or k1 < lane_image_gray_width * 0.75:
                departure_count = departure_count + 1

    # 차선이탈판단 조건 만족시 경고 출력
    if departure_count >= 3:
        cv2.putText(source, 'Lane departure are detected.',
                    (int(cap_image_width * 0.25), int(cap_image_height * 0.1)), font, 1, (50, 50, 255), 1)

    # 핸들이미지 조건 만족시 차선의 기울기에 따라 핸들의 회전된 이미지 출력
    if count_lane_left > 20 or count_lane_right > 20:
        Draw_hough_line_image(source, line_image_left, line_image_right, count_lane_left, count_lane_right)
    else:
    ...
    cv2.line(return_image, (int(lane_image_gray_width * 0.25), 0)
             , (int(lane_image_gray_width * 0.25), lane_image_gray_height), (0, 0, 255), 2)
    cv2.line(return_image, (int(lane_image_gray_width * 0.75), 0)
             , (int(lane_image_gray_width * 0.75), lane_image_gray_height), (0, 0, 255), 2)
...
def Draw_hough_line_image(source, line_image_left, line_image_right, count_lane_left, count_lane_right):
    global font
    line_image = np.zeros((line_image_left.shape[0], line_image_left.shape[1], 3), line_image_left.dtype)

    line_image_left = cv2.cvtColor(line_image_left, cv2.COLOR_BGR2GRAY)
    line_image_right = cv2.cvtColor(line_image_right, cv2.COLOR_BGR2GRAY)
    ...
    if lines_left is not None and lines_right is not None and count_lane_left > 20 and count_lane_right > 20:
        lines = np.concatenate((lines_left, lines_right), axis=0)
        Draw_line(source, lines, line_image)
    elif lines_left is not None and count_lane_left > 20:
        lines = lines_left
        Draw_line(source, lines, line_image)
    elif lines_right is not None and count_lane_right > 20:
        lines = lines_right
        Draw_line(source, lines, line_image)


def Draw_line(source, lines, line_image):
    global p1_handle_angle
    global p2_handle_angle
    if lines is not None:
        for i in range(0, len(lines)):
            ...
            pt2_x = int(x0 - 1000 * (-b))
            pt2_y = int(y0 - 1000 * a)
            cv2.line(line_image, (pt1_x, pt1_y), (pt2_x, pt2_y), 255, 1, 8)

        # 핸들 이미지를 주행 영상에 합성하기위한 사전작업
        if lines[0][0][0] < -0.0:
            angle = - (1 / lines[0][0][0] * 300)
            M = cv2.getRotationMatrix2D((handle_image_height / 2, handle_image_width / 2),
                                        (angle + p1_handle_angle + p2_handle_angle) / 3, 1)
            p2_handle_angle = p1_handle_angle
            p1_handle_angle = angle
        elif lines[len(lines) - 1][0][0] > 0.0:
            angle = - (1 / lines[len(lines) - 1][0][0] * 300)
            M = cv2.getRotationMatrix2D((handle_image_height / 2, handle_image_width / 2),
                                        (angle + p1_handle_angle + p2_handle_angle) / 3, 1)
            p2_handle_angle = p1_handle_angle
            p1_handle_angle = angle
        ...
        handle_image_inverted = cv2.bitwise_not(handle_image)
        turned_handle_image = cv2.warpAffine(handle_image_inverted, M, (handle_image_height, handle_image_width))

        _, mask_image = cv2.threshold(turned_handle_image[:, :], 50, 255, cv2.THRESH_BINARY)
        ...
        dst_inverted = cv2.bitwise_not(mask_image)
        turned_handle_image = cv2.bitwise_and(mask_image, turned_handle_image)
        turned_handle_image = cv2.normalize(turned_handle_image, None, 0, 220, cv2.NORM_MINMAX)
        bg = cv2.bitwise_and(roi, dst_inverted)
        added = cv2.bitwise_xor(turned_handle_image, bg)
        added = cv2.cvtColor(added, cv2.COLOR_GRAY2BGR)
